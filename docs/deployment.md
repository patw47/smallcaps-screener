...
| `DEMO_MODE` | No (default off) | Forces presentation mode on every `/api/scan` response (no rule thresholds served, redacted glossary). Set it on any instance a third party can reach: `?demo=1` can then only be redundant, never lifted. |
