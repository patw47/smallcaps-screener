...
## Les seuils ne sont jamais affichés

Le tableau de bord ne montre **aucune valeur de règle**, et ce n'est pas un mode qu'on
active : c'est son seul comportement. Un masquage optionnel est un masquage qu'on oublie —
l'écran est déjà affiché quand on y pense.
...
- les infobulles et les paragraphes sont écrits sans leur chiffre, en gardant la
  justification mesurée ;
- le point de contrôle du suivi affiche son échéance sans son seuil ;
- le calendrier d'observation, lui, reste visible : c'est un protocole de mesure, pas un
  critère de sélection.

Les valeurs et leurs justifications complètes vivent dans la page de référence privée, hors
application (`docs/criteria-index.md` en porte le lien).
